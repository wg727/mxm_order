    ...
    methods: {
        ...
        getProducts() {
            if (this.addInfo.assetsOrgId === undefined) {
                ...
                newProductList({orgId:this.addInfo.assetsOrgId}).then(res=>{
                    ...
                    this.productList=JSON.parse(JSON.stringify(res.data).replace(/name/g, 'productName').replace(/id/g, 'productId').replace(/orgId/g, 'outOrgId')) 
                    })
        ...
        addShipment(formName) {
            ...
            this.addInfo.applyItemList.forEach((item, index) => {
                // let outOrg = this.addInfo.assetsOrgId; //调出组织
                let applyNum = this.addInfo.applyItemList[index].applyNum;//申请数量
                let auditNums = this.addInfo.applyItemList[index].applyNum;
                let productSpecId='';//产品规格id
                let specUnionKey='';//产品规格
                (item.applyNum = applyNum), (item.auditNum = auditNums);
            });
