    ...
    <div>
        ...
        <el-dialog title="产品列表" :visible.sync="addProductVisible" width="50%" :before-close="handleClose">
            <div class="addProductVisible">
                ...
                <el-row :gutter="10">
                    <el-col :span="7">
                        <el-input placeholder="请输入产品名称" @clear="getProducts()" clearable v-model="inputproductName" class="search_input"></el-input>
    ...
    methods: {
        ...
        getProducts() {
            ...
            productLists(requertProList).then(res => {
                ...
                // this.addInfo.applyItemList=res.data.productId;
                // this.addInfo.productSpecId=res.data.productSpecId;
                this.producTableInfo.producTablelList.productName=res.data.productName;
                this.producTableInfo.producTablelList.specUnionKey=res.data.specUnionKey;
                this.producTableInfo.producTablelList.unitText=res.data.unitText;
        ...
        handleSelectionChange(val) {
            ...
            // var datas=val
            // for(var i in datas){
            //     val[i].outOrgId=this.addInfo.assetsOrgId  //调出组织
            //     val[i].applyNum=this.addInfo.applyNum[i].applyNum  //申请数量
            //     val[i].auditNum=this.addInfo.applyNum[i].applyNum  // 审批数量
            //     val[i].bathEnable=0  //是否启用批次管理
            //     val[i].serialNumberEnable=0  //是否启用序列号管理（0否1是
            // }
            this.addInfo.applyItemList=datas
            // for (let i = 0; i < val.length; i++) {
                // this.addInfo.applyItemList[i].productName = val[i].productName;
                // this.addInfo.applyItemList[i].specUnionKey = val[i].specUnionKey;
                // this.addInfo.applyItemList[i].unitText = val[i].unitText;
                // this.addInfo.applyItemList[i].idleQuantity = val[i].idleQuantity;
                // this.addInfo.applyItemList[i].applyNum = val[i].applyNum;
                

                // this.addInfo.applyItemList[i].push(val[i].productName)
                // this.addInfo.applyItemList[i].push(val[i].specUnionKey)
                // this.addInfo.applyItemList[i].push(val[i].unitText)
                // this.addInfo.applyItemList[i].push(val[i].idleQuantity)
                // this.addInfo.applyItemList[i].push(val[i].applyNum)
            // }
